...
contract BootstrapTest is Test {
    ...
    function setUp() public {
        ...
        vm.chainId(1);
        // non pectra mode
        uint256 pectraTs = NetworkConstants.getPectraHardForkTimestamp();
        address toEtch = NetworkConstants.getDepositContractAddress();
        // etching cannot be the creationCode
        vm.etch(toEtch, vm.getDeployedCode("test/foundry/unit/Bootstrap.t.sol:ETHPOSDepositMock"));
        vm.warp(pectraTs - 1);
    ...
    function test24_Stake_Pectra_Range() public {
        uint256 pectraTime = NetworkConstants.getPectraHardForkTimestamp();
        vm.warp(pectraTime + 1);

        uint256 inRange = 33 ether;
        ...
        bytes32 depositDataRoot = bytes32(0);
        vm.startPrank(addrs[0]);
        vm.deal(addrs[0], inRange);
        bootstrap.stake{value: inRange}(pubkey, signature, depositDataRoot);
        IImuaCapsule capsule = bootstrap.ownerToCapsule(addrs[0]);
        address expectedCapsuleAddress = Create2.computeAddress(
            bytes32(uint256(uint160(addrs[0]))),
            keccak256(abi.encodePacked(BEACON_PROXY_BYTECODE, abi.encode(address(capsuleBeacon), ""))),
        ...
        assertEq(address(capsule), expectedCapsuleAddress, "Capsule address does not match expected");
        vm.stopPrank();
        // check that all the value was staked and not left behind
        assertEq(address(bootstrap).balance, 0);
    }

    function test24_Stake_Pectra_Range_Lower() public {
        uint256 pectraTime = NetworkConstants.getPectraHardForkTimestamp();
        vm.warp(pectraTime + 1);

        uint256 lower = 32 ether - 1;
        ...
        bytes32 depositDataRoot = bytes32(0);
        vm.startPrank(addrs[0]);
        vm.deal(addrs[0], lower);
        vm.expectRevert(Errors.NativeRestakingControllerInvalidStakeValue.selector);
        bootstrap.stake{value: lower}(pubkey, signature, depositDataRoot);
        vm.stopPrank();
    }

    function test24_Stake_Pectra_Range_Higher() public {
        uint256 pectraTime = NetworkConstants.getPectraHardForkTimestamp();
        vm.warp(pectraTime + 1);

        uint256 higher = 2048 ether + 1;
        ...
        bytes32 depositDataRoot = bytes32(0);
        vm.startPrank(addrs[0]);
        vm.deal(addrs[0], higher);
        vm.expectRevert(Errors.NativeRestakingControllerInvalidStakeValue.selector);
        bootstrap.stake{value: higher}(pubkey, signature, depositDataRoot);
        vm.stopPrank();
    }

    function test24_Stake_Pectra_AtLowerBound32() public {
        uint256 pectraTime = NetworkConstants.getPectraHardForkTimestamp();
        vm.warp(pectraTime + 1);
        _enableNativeRestaking();
        vm.startPrank(addrs[0]);
        vm.deal(addrs[0], 32 ether);
        bootstrap.stake{value: 32 ether}(new bytes(48), new bytes(96), bytes32(0));
        vm.stopPrank();
        assertEq(address(bootstrap).balance, 0);
    }

    function test24_Stake_Pectra_AtUpperBound2048() public {
        uint256 pectraTime = NetworkConstants.getPectraHardForkTimestamp();
        vm.warp(pectraTime + 1);
        _enableNativeRestaking();
        vm.startPrank(addrs[0]);
        vm.deal(addrs[0], 2048 ether);
        bootstrap.stake{value: 2048 ether}(new bytes(48), new bytes(96), bytes32(0));
        vm.stopPrank();
        assertEq(address(bootstrap).balance, 0);
    }
