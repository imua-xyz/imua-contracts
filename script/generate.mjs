...
async function updateGenesisFile() {
  try {
    ...
    if (!genesisJSON.app_state.assets.tokens) {
      // Initialize the tokens array with a reserved placeholder at index 0
    ...
    for (let i = 0; i < finalOracleTokens.length; i++) {
      ...
      const priceEntry = {
        ...
        token_id: (i + 1).toString() // token_id starts from 1
      };
