...
const DEFAULT_CONFIG = {
  ...
  chains: {
    ...
    xrp: {
      ...
      tempOutput: 'genesis/xrp_bootstrap_genesis.json',
      useTsx: true,
    }
  },
  merge: {
  ...
  async runChainGenesis(chainName, chainConfig, options = {}) {
    try {
      ...
      if (chainName === 'Bitcoin') {
        const oldValue = process.env.BITCOIN_GENESIS_OUTPUT_PATH;
        process.env.BITCOIN_GENESIS_OUTPUT_PATH = resolvedOutputPath;
        console.log(`🔧 Setting BITCOIN_GENESIS_OUTPUT_PATH=${resolvedOutputPath} (was: ${oldValue || 'undefined'}) for Bitcoin genesis`);
      }
      ...
      if (chainName === 'XRP') {
        ...
        console.log(`🔧 Setting XRP_GENESIS_OUTPUT_PATH=${resolvedOutputPath} (was: ${oldValue || 'undefined'}) for XRP genesis`);
      }
...
(async () => {
  ...
  /**
   * Run XRP genesis generation script
   * @param {Object} chainConfig - Chain configuration
   * @returns {Object} Generated genesis state
   */
  async runXrpGenesis(chainConfig) {
    // Use absolute path to ensure consistency between read and write operations
    const absoluteOutputPath = path.resolve(process.cwd(), 'genesis/xrp_bootstrap_genesis.json');

    return this.runChainGenesis('XRP', chainConfig, {
      defaultOutputPath: absoluteOutputPath,
 ...
 */
  validateGenesisStructure(genesis) {
    ...
    if (missingModules === 0 && uniqueModules.length > 0) {
      console.log(`✅ All ${uniqueModules.length} expected modules found in genesis - Genesis structure validation completed`);
    } else if (uniqueModules.length > 0) {
      console.log(`⚠ Genesis validation completed with ${missingModules} missing expected modules - Genesis structure validation completed`);
    } else {
    ...
    try {
      ...
      const chainLoaders = [
        ...
        },
        {
          ...
          defaultPath: config.chains.xrp?.tempOutput || 'genesis/xrp_bootstrap_genesis.json'
        }
      ];
